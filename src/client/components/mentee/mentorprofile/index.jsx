...
//                 {mentorData.about || <p style={{ color: "#777", fontSize: "0.87rem" }}>No bio available.</p>}
...
const MentorProfile = () => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      // Replace the renderTabContent function's "about" case with this fixed version
            ...
            {posts.length === 0 ? (
              ...
              posts.map((post) => (
                <div key={post.id} style={{
                  ...
                  {/* Comments section - keeping your existing comment logic */}
                  {openComments[post.id]?.open && (
                    <>
                      <div style={{ marginTop: "1rem" }}>
                        {Array.isArray(post.comments) && [...post.comments]
  ...
  const bookingCard = (
    ...
    >
      ...
      {/* <button
        onClick={handlePayNow}
        disabled={currentUser?.mentor_id === mentorData?.id}   // 🔹 disable if same mentor
        style={{
          marginTop: "1.5rem",
          width: "100%",
          padding: "12px",
          backgroundColor: currentUser?.mentor_id === mentorData?.id ? "#ccc" : "#198754",
          color: "white",
          border: "none",
          borderRadius: "6px",
          fontWeight: "600",
          fontSize: "1rem",
          cursor: currentUser?.mentor_id === mentorData?.id ? "not-allowed" : "pointer",
        }}
      >
        💳 Pay Now
      </button> */}
      {/* <button
        onClick={() => {
          if (!currentUser) {
            ...
            // OR, if you have a login modal:
            // setShowLoginModal(true);
            return;
          }

          if (currentUser?.mentor_id === mentorData?.id) {
            return; // prevent mentor booking themselves
          }

          handlePayNow(); // proceed to payment
        }}
        style={{
          marginTop: "1.5rem",
          width: "100%",
